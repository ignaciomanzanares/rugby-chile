...
function Section({ icon: Icon, title, children }: { icon: React.ElementType; title: string; children: React.ReactNode }) {
  return (
    <section className="rounded-xl border border-border bg-card/40 p-5 md:p-6">
    ...
    <div className="min-h-screen bg-background text-foreground">
      <section className="border-b border-border bg-card/50">
        <div className="container mx-auto px-4 py-8">
          ...
          <p className="text-muted-foreground text-sm">Primera División · Acuerdos de Participación ARUSA 2026</p>
        </div>
      </section>
      ...
      <div className="container mx-auto px-4 py-8 space-y-6 max-w-5xl">
        ...
        <Section icon={Users} title="Equipos participantes (10)">
          ...
          <div className="grid sm:grid-cols-2 gap-2">
            {TEAMS.map((t) => {
              ...
              return (
                <div key={t.name} className="flex items-center gap-3 px-3 py-2.5 rounded-lg border border-border bg-card/50">
                  {logo ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={logo} alt={t.name} className="w-8 h-8 rounded-full object-cover flex-shrink-0 ring-1 ring-border" />
                  ) : (
                    <span
                  ...
                  <span className={`text-[10px] font-bold tracking-wider uppercase px-2 py-0.5 rounded ${t.kind === "Invitado" ? "bg-amber-500/15 text-amber-400" : "bg-muted text-muted-foreground"}`}>
                    {t.kind}
                  </span>
                </div>
              );
        ...
        <Section icon={Shield} title="Tres equipos por club">
          ...
          <div className="grid md:grid-cols-3 gap-3">
            <div className="rounded-lg border border-red-600/30 bg-red-600/10 p-4">
              ...
              <p className="text-xs text-muted-foreground mt-2">Primer equipo · titulares · 80 min (2×40&apos;)</p>
            </div>
            <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-4">
              ...
              <p className="text-xs text-muted-foreground mt-2">Segundo equipo · 2 hs antes de Primera</p>
            </div>
            <div className="rounded-lg border border-emerald-600/30 bg-emerald-600/10 p-4">
              ...
              <p className="text-xs text-muted-foreground mt-2">Tercer equipo · 2 hs antes de Intermedia</p>
            </div>
          </div>
          <div className="mt-4 grid sm:grid-cols-2 gap-3 text-xs text-muted-foreground">
            <div className="rounded-lg border border-border px-3 py-2.5">
              <span className="text-muted-foreground">Plantel mínimo Primera:</span> <span className="text-foreground font-semibold">80 jugadores licenciados</span>
            </div>
            <div className="rounded-lg border border-border px-3 py-2.5">
              <span className="text-muted-foreground">Repetición entre equipos:</span> <span className="text-foreground font-semibold">máx. 8 jugadores por fecha</span>
            </div>
          </div>
        </Section>
        ...
        <Section icon={Calendar} title="Formato del torneo">
          <div className="grid md:grid-cols-3 gap-3">
            <div className="rounded-lg border border-border bg-card/60 p-4">
              ...
              <p className="text-xs text-muted-foreground uppercase tracking-wider mt-1">Fechas de fase regular</p>
            </div>
            <div className="rounded-lg border border-border bg-card/60 p-4">
              <div className="text-3xl font-black text-foreground">10</div>
              <p className="text-xs text-muted-foreground uppercase tracking-wider mt-1">Equipos · todos contra todos</p>
            </div>
            <div className="rounded-lg border border-border bg-card/60 p-4">
              <div className="text-3xl font-black text-foreground">4</div>
              <p className="text-xs text-muted-foreground uppercase tracking-wider mt-1">Clasifican a playoffs</p>
            </div>
          </div>
        ...
        <Section icon={Trophy} title="Sistema de puntos">
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-2">
            {[
              ...
              <div key={i} className="flex items-center gap-3 rounded-lg border border-border bg-card/40 px-3 py-2.5">
        ...
        <Section icon={Trophy} title="Playoffs">
          <div className="space-y-2">
            <div className="rounded-lg border border-border bg-card/50 px-4 py-3 flex items-center gap-3">
              ...
              <span className="text-muted-foreground/70 text-xs">vs</span>
        ...
        <Section icon={ArrowUp} title="Ascenso, descenso y repechaje">
          <div className="grid md:grid-cols-3 gap-3">
            <div className="rounded-lg border border-emerald-600/30 bg-emerald-600/10 p-4">
              ...
              <p className="text-sm text-foreground/80">Los <span className="font-bold text-foreground">4 primeros</span> de la fase regular juegan playoffs.</p>
            </div>
            <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-4">
              <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-sm text-foreground/80">El <span className="font-bold text-foreground">9°</span> de Primera enfrenta al campeón de Segunda.</p>
            </div>
            <div className="rounded-lg border border-red-700/40 bg-red-700/10 p-4">
              ...
              <p className="text-sm text-foreground/80">El <span className="font-bold text-foreground">10°</span> baja a Segunda División en 2027.</p>
            </div>
          </div>
        ...
        <Section icon={Clock} title="Reglas de partido">
          <div className="grid sm:grid-cols-2 gap-3 text-sm">
            <div className="rounded-lg border border-border bg-card/40 px-4 py-3">
              <p className="text-muted-foreground text-xs uppercase tracking-wider">Tiempo de juego</p>
              <p className="font-semibold mt-1">80 min · 2 × 40 + 10 min descanso</p>
            </div>
            <div className="rounded-lg border border-border bg-card/40 px-4 py-3">
              <p className="text-muted-foreground text-xs uppercase tracking-wider">Match Team Sheet</p>
              <p className="font-semibold mt-1">33 personas · 23 jugadores (15 + 8) + staff</p>
            </div>
            <div className="rounded-lg border border-border bg-card/40 px-4 py-3">
              <p className="text-muted-foreground text-xs uppercase tracking-wider">Sustituciones · Primera</p>
              <p className="font-semibold mt-1">Máximo 8 (según Regulación 3 World Rugby)</p>
            </div>
            <div className="rounded-lg border border-border bg-card/40 px-4 py-3">
              <p className="text-muted-foreground text-xs uppercase tracking-wider">Sustituciones · Intermedia y Pre-Intermedia</p>
              <p className="font-semibold mt-1">Rotativas ilimitadas</p>
            </div>
            <div className="rounded-lg border border-border bg-card/40 px-4 py-3">
              <p className="text-muted-foreground text-xs uppercase tracking-wider">Balón oficial</p>
              <p className="font-semibold mt-1">Simoon Ball N° 5 Macron</p>
            </div>
            <div className="rounded-lg border border-border bg-card/40 px-4 py-3">
              <p className="text-muted-foreground text-xs uppercase tracking-wider">W.O.</p>
        ...
        <Section icon={AlertTriangle} title="Disciplina">
          <ul className="space-y-2 text-sm text-foreground/80">
            <li className="flex gap-3"><span className="text-red-500 font-bold w-5">·</span><span><span className="font-semibold text-foreground">Doble amarilla:</span> equivale a roja y suspensión automática para la siguiente fecha.</span></li>
            <li className="flex gap-3"><span className="text-red-500 font-bold w-5">·</span><span><span className="font-semibold text-foreground">3 amarillas acumuladas:</span> suspensión 1 fecha. El contador se reinicia. Las amarillas también se resetean al iniciar los playoffs.</span></li>
            <li className="flex gap-3"><span className="text-red-500 font-bold w-5">·</span><span><span className="font-semibold text-foreground">Tarjeta roja:</span> el jugador queda suspendido hasta la resolución del Tribunal de Disciplina.</span></li>
            <li className="flex gap-3"><span className="text-red-500 font-bold w-5">·</span><span><span className="font-semibold text-foreground">Protocolo &ldquo;Identifique y Retire&rdquo;:</span> sospecha de conmoción cerebral retira al jugador del partido. Reintegro gradual mínimo de 21 días (salvo apto neurológico).</span></li>
            <li className="flex gap-3"><span className="text-red-500 font-bold w-5">·</span><span><span className="font-semibold text-foreground">Alineación indebida:</span> denuncia hasta 6 días corridos posteriores al encuentro. Sanción: W.O. administrativo 28-0.</span></li>
          </ul>
        </Section>

        <p className="text-xs text-muted-foreground/70 text-center pt-2">
          Documento oficial: <span className="text-muted-foreground">Acuerdos de Participación ARUSA 2026</span> · Datos del torneo en{" "}
          <a href="https://arusa.cl/en/tournament/1328550/summary" target="_blank" rel="noopener noreferrer" className="hover:text-foreground/80 underline underline-offset-2 transition-colors">arusa.cl</a>
        </p>
      </div>
    </div>
  );
