...
import { clubLogo, type StandingRow } from "@/lib/tournament";
import { useLeveradeStandings } from "@/lib/use-leverade-standings";

export function TeamsGrid({ initialRows }: { initialRows?: StandingRow[] | null }) {
  // Posición + puntos desde la tabla real de Primera. Se siembra en el server
  // (initialRows) para que salga la data real en el primer render — sin flash
  // del snapshot viejo y sin placeholder eterno — y se mantiene fresca al vuelo.
  const { rows: live } = useLeveradeStandings("PRIMERA", initialRows);
